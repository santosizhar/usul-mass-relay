...
Artifacts:
- Run record TypeScript interface in `packages/run/src/run.ts`.
- Run JSON schema in `packages/run/src/run.schema.json`.
...
Artifacts:
- File-based Run persistence helpers in `packages/run/src/run-store.ts`.
- JSONL export helper in `packages/run/src/run-exporter.ts`.
