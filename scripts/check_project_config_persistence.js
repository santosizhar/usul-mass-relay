...
const schemaPath = path.join(
  ...
  "src",
  "project-config-persistence.schema.json"
);
const samplePath = path.join(
  repoRoot,
  "artifacts",
  "examples",
  "project.config.persistence.sample.json"
);
...
validate(schema, sample, "project.config.persistence");

const projectId = sample.project_id;
if (typeof projectId === "string") {
  const expectedSuffix = `${projectId}/config.json`;
  if (!sample.storage?.object_path?.endsWith(expectedSuffix)) {
    errors.push("storage.object_path must end with <project_id>/config.json");
  }
}
...
if (errors.length > 0) {
  console.error("Project config persistence checks failed:");
  errors.forEach((error) => console.error(`- ${error}`));
  process.exit(1);
}

console.log("Project config persistence checks passed.");
